...
export interface IArgument extends Locatable {
	type: typeof ARG;
	actual: SyntaxNode;
	keyword?: SyntaxNode;
	loop?: CompFor;
 ...
 */
export function walk(node: SyntaxNode, walkListener?: WalkListener): SyntaxNode[] {
 ...
 */
function walkRecursive(node: SyntaxNode, ancestors?: SyntaxNode[], walkListener?: WalkListener): SyntaxNode[] {
	...
	switch (node.type) {
		...
		case RETURN: children = node.values ? node.values : []; break;
