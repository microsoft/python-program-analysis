...
// strings
stringliteral           ([rR]|[uU]|[fF]|[fF][rR]|[rR][fF])?({shortstring})
...
bytesliteral            {bytesprefix}({shortstring})
...
<INITIAL,INLINE>{tripledquote}  %{
                            tripleString = "";
                            this.begin( 'TRIPLE_D' )
                        %}
<TRIPLE_D>{tripledquote} %{
                        ...
                        %}
<TRIPLE_D>\n            tripleString += "\n";
<TRIPLE_D>.             tripleString += yytext;

<INITIAL,INLINE>{triplesquote}  %{
                            tripleString = "";
                            this.begin( 'TRIPLE_S' )
                        %}
<TRIPLE_S>{triplesquote} %{
                            // escape string and convert to double quotes
                            // http://stackoverflow.com/questions/770523/escaping-strings-in-javascript
                            var str = tripleString
                                .replace( /[\\"']/g, '\\$&' )
...
<INLINE>{operators}     %{
                            ...
                            return yytext 
                        %}
<INLINE>{stringliteral} return 'STRING'
<INLINE>{bytesliteral} %{
                            // escape string and convert to double quotes
                            // http://stackoverflow.com/questions/770523/escaping-strings-in-javascript
...
yield_expr
    : 'yield'
        { $$ = { type: 'yield', location: @$ } }
    | 'yield' 'from' test
        { $$ = { type: 'yield', from: $3, location: @$ } }
    | 'yield' testlist
        { $$ = { type: 'yield', value: $2, location: @$ } }
    ;
